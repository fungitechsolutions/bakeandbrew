...
function UsersErrorContent({
  message,
  onRetry,
}: Pick<UsersErrorStateProps, "message" | "onRetry">) {
  return (
    <div className="flex min-h-64 flex-col items-center justify-center gap-6 border border-[rgba(47,78,64,0.18)] bg-white px-6 py-16 text-center">
      ...
      <div className="flex flex-wrap items-center justify-center gap-3">
        {onRetry && (
          <button type="button" onClick={onRetry} className={adminPrimaryButtonClass}>
            <RotateCcw size={12} />
            Retry
          </button>
        )}
...
export function UsersErrorState({
  ...
  if (variant === "inline") {
    return <UsersErrorContent message={message} onRetry={onRetry} />;
  }
  ...
  return (
    <AdminPageLayout
      ...
      maxWidth="wide"
    >
      <UsersErrorContent message={message} onRetry={onRetry} />
    </AdminPageLayout>
  );
}
