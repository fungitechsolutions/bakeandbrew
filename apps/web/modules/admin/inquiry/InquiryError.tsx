...
function InquiryErrorContent({
  message,
  code,
  onRetry,
}: Pick<Props, "message" | "code" | "onRetry">) {
  const router = useRouter();
  const hint = code ? (CODE_HINTS[code] ?? null) : null;

  return (
    <div className="flex min-h-64 flex-col items-center justify-center border border-[rgba(47,78,64,0.18)] bg-white px-6 py-16 text-center">
      <div className="mb-6 flex h-14 w-14 items-center justify-center border border-[rgba(194,138,79,0.3)] bg-[rgba(194,138,79,0.08)]">
        <AlertTriangle
          className="h-7 w-7 text-(--brand-brown)"
          strokeWidth={1.5}
        />
      </div>
...
export default function InquiryError({
  ...
  if (variant === "inline") {
    return (
      <InquiryErrorContent message={message} code={code} onRetry={onRetry} />
    );
  }
